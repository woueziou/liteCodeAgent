...
4. Invoke `dispatcher` (via the `task` tool, targeting the `dispatcher` subagent) scoped to the newly created issue — same pattern as `chained-implementation`'s dispatcher step: tell it explicitly which issue to plan, not to run a full-backlog ranking pass.
