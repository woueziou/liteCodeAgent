...
3. If the ticket was successfully moved to `Planned` (or was already there), invoke `implementer` (via the `task` tool, targeting the `implementer` subagent) on that ticket.
