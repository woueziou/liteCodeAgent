...
8. Invoke **both** review passes on the PR/diff, each via `task` — they're independent, so issue both calls together: `reviewer` (subagent_type `reviewer`: plan fidelity, conventions, verification, attribution) and `bug-hunter` (subagent_type `bug-hunter`: the correctness pass — concrete failure scenarios, confirmed by running the code; per ADR 0013 it replaces the `code-review` sub-pass `reviewer` used to invoke). Give both the PR, the base and the branch; give `reviewer` the plan too. Neither is optional; do not move the ticket's status at all without both reports in hand. These calls **block**: do not end your turn or report a final `STATUS:` until both `task` calls have actually returned their text to you. If you find yourself about to say something like "I'll wait for the reviewer's notification" — stop, that means you are treating an in-turn tool call as if it were an external async event it is not; the `task` call already gives you the full report synchronously when it returns. Read both, then continue immediately to steps 9-10 in the same run.
...
- You never claim to have invoked `reviewer` or `bug-hunter` without an actual `task` call and a real report back — no simulating their output yourself.
