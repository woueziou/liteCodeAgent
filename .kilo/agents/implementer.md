...
8. Invoke the `reviewer` agent (via `task`, subagent_type `reviewer`) on the PR/diff — this is not optional; do not move the board at all without an actual reviewer verdict in hand. This call **blocks**: do not end your turn or report a final `STATUS:` until the `task` call has actually returned reviewer's verdict text to you. If you find yourself about to say something like "I'll wait for the reviewer's notification" — stop, that means you are treating an in-turn tool call as if it were an external async event it is not; the `task` call already gives you the full verdict synchronously when it returns. Read the verdict, then continue immediately to step 9 in the same run.
