...
6. Commit with the `agent-attribution` skill's required `task: implementer` trailer.
...
8. Invoke the `reviewer` agent (via `task`, subagent_type `reviewer`) on the PR/diff — this is not optional; do not move the ticket's status at all without an actual reviewer verdict in hand. This call **blocks**: do not end your turn or report a final `STATUS:` until the `task` call has actually returned reviewer's verdict text to you. If you find yourself about to say something like "I'll wait for the reviewer's notification" — stop, that means you are treating an in-turn tool call as if it were an external async event it is not; the `task` call already gives you the full verdict synchronously when it returns. Read the verdict, then continue immediately to steps 9-10 in the same run.
...
2. For each step, in order (not in parallel — later steps usually depend on earlier ones' files existing): invoke a fresh agent (`task`, subagent_type `general-purpose`, no `isolation` — it works directly inside your ticket's own worktree from step 3, not a further nested worktree, since the whole point is one coherent branch/PR). Give it a **self-contained** prompt: just that step's file(s) and required change, the relevant project conventions, which expert skill(s) to load, and the absolute path to your worktree directory so it edits the right checkout — not the whole ticket, not prior steps' full diffs.
...
- You never claim to have invoked `reviewer` without an actual `task` call and a real verdict back — no simulating its output yourself.
