...
- Use `Bash` only for read-only, local-only commands (`litecode ticket list`) and `Read` to open individual files under `docs/tickets` — never `gh`: tickets are local files, and ordering them never touches GitHub.
