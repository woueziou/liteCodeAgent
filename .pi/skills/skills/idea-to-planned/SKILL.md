...
4. Invoke `dispatcher` (via `litecode run dispatcher --prompt-file <file>` via `Bash`) scoped to the newly drafted ticket — same pattern as `chained-implementation`'s dispatcher step: tell it explicitly which ticket to plan, not to run a full-backlog ranking pass.
