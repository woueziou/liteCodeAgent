...
1. Invoke `dispatcher` (via `litecode run dispatcher --prompt-file <file>` via `Bash`) scoped to the named issue only — tell it explicitly which issue to plan, not to run its normal full-backlog ranking pass, so it doesn't pull in unrelated items.
...
3. If the issue was successfully moved to `Planned` (or was already there), invoke `implementer` (via `litecode run implementer --prompt-file <file>` via `Bash`) on that issue number.
