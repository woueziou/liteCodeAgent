...
3. If the ticket was successfully moved to `Planned` (or was already there), invoke `implementer` (via `litecode run implementer --prompt-file <file>` via `Bash`) on that ticket.
