...
À trancher : peut-on vérifier l'invariant ailleurs que dans les prompts ? Pistes non pré-décidées — un hook `PreToolUse` sur `Bash` refusant `gh project` sauf pour `sync` ; un wrapper `gh` dans le PATH des agents ; une détection a posteriori dans le rapport d'agent. Noter que le hook vit dans le `.claude/settings.json` du projet appelant, que ce CLI ne contrôle pas (même raisonnement que l'ADR 0009, décision 1).
