...
7. Push your feature branch and open a PR — target whichever branch you actually based the worktree on in step 3 (`--base main` for the normal case, `--base <pr-branch-name>` for a follow-on branch): `gh pr create --repo woueziou/liteCodeAgent --title "..." --body-file <path> --base <base-branch>`, with a body that names the ticket (`Ticket: <NNNN-slug>`, path to its file) — there is no issue to close.
...
2. For each step, in order (not in parallel — later steps usually depend on earlier ones' files existing): invoke a fresh agent via the `Agent` tool (subagent_type `general-purpose`) — it works directly inside your ticket's own worktree from step 3, not in a further isolated or nested worktree of its own, since the whole point is one coherent branch/PR. Give it a **self-contained** prompt: just that step's file(s) and required change, the relevant project conventions, which expert skill(s) to load, and the absolute path to your worktree directory so it edits the right checkout — not the whole ticket, not prior steps' full diffs.
