...
1. Invoke `orchestrator` (via the `Agent` tool (subagent_type `orchestrator`)) with the raw idea, exactly as you would for a normal recommendation request.
2. Read its output.
...
3. Invoke `tracker` (via the `Agent` tool (subagent_type `tracker`)) with the title/body/label/size/priority derived from `orchestrator`'s output — same as the manual flow, just without a separate confirmation round-trip first. Populate the ticket body with the plan/recommendation `orchestrator` returned, same as when a human manually asks for a ticket to be created.
4. Invoke `dispatcher` (via the `Agent` tool (subagent_type `dispatcher`)) scoped to the newly drafted ticket — same pattern as `chained-implementation`'s dispatcher step: tell it explicitly which ticket to plan, not to run a full-backlog ranking pass.
