...
test("a missing agentSkills key is reported with the referencing pack file", async () => {
  ...
  expect(missing.map((m) => m.path)).toContain("project.agentSkills.sync");
  const entry = missing.find((m) => m.path === "project.agentSkills.sync")!;
  expect(entry.sources.some((s) => s.includes("agents/sync.md"))).toBe(true);
});

test("a present-but-empty agentSkills key is not reported as missing", async () => {
  const config = await exampleConfig();
  (config.project.agentSkills as Record<string, string[]>).sync = [];
...
test("computeAgentSkillsFix derives values for exactly the missing keys, nothing else", async () => {
  ...
  expect(Object.keys(fix)).toEqual(["sync"]);
  expect(fix.sync).toEqual(before.sync);
});
