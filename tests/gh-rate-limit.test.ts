...
import { gh, RateLimitError } from "../src/gh.ts";
...
`);
  ...
  const out = await gh(["pr", "view", "7", "--json", "state"]);
  expect(JSON.parse(out).data.ok).toBe(true);
...
`);
  ...
  const err = await gh(["pr", "view", "7"]).catch((e: unknown) => e);
...
`);
  ...
  const err = await gh(["pr", "view", "9999"]).catch((e: unknown) => e);
  expect((err as Error).message).toContain("Could not resolve");
  expect((err as Error).message).toContain("gh pr view 9999 failed");
