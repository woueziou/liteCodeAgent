...
Write the prose inside `PLAN:` and `RECOMMENDATION:` in {{ project.language }}. Everything else in the output structure below stays in English: the `SIZE:`/`PANEL:`/`ADR:` sentinel keys and their enum values, and any `BLOCKING_TENSION`/panel-member text you relay verbatim rather than author yourself.
{{/if}}
...
ADR: <path under {{#if project.adrDir}}{{ project.adrDir }}/{{/if}}{{^if project.adrDir}}n/a — this project does not use ADRs{{/if}}, or "none (trivial, no ADR)", or "none (panel degraded)">
