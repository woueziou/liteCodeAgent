...
The ticket file is the ticket's whole history — there is no issue to comment on. Whenever this page says to leave a **note on the ticket**, append it to the end of the ticket file's body with `Edit`/`Write`, under a heading of its own: `### <YYYY-MM-DD> — implementer: <what this note is>`. Never rewrite or delete what's already there: earlier notes are the record of how the ticket got here. Write notes — and every `status` change — in the **main checkout's** copy of the ticket file, the one step 2 edits, so `dispatcher`, `triage`, the dashboard and `ticket list` see them immediately. Never commit a ticket file yourself, and never edit it in your worktree: committing ticket files is the human's call (ADR 0015), and a second copy on your branch would only diverge.
...
2. Move the ticket from `Planned` to `In Progress` before writing any code: set its `status` field to `inProgress` with `Edit`/`Write`. The ticket file is the only place pipeline state lives (ADR 0012, ADR 0015), so that write *is* the move. Verify it actually landed by re-reading the file — don't just trust the tool call.
...
5. If you're resumed specifically to continue past this gate — whether by the same agent process or, more commonly, by a **freshly invoked `implementer`** with no memory of this run (the normal case: the underlying agent/transcript is ephemeral and a same-process resume is not reliable — do not assume it will work, and do not treat a `could not be resumed` error as a blocker to escalate, it's the expected path here) — do not have the calling session reconstruct worktree/branch/commit/ADR-path/actions-done from memory or from re-reading the whole thread. Instead, reconstruct mechanically from the durable artifact:
   ...
   - Read the ADR draft note on the ticket file and its `resume-manifest` block. Treat the human's approval message as authorization to act on exactly what that manifest says — not on whatever the caller happens to paraphrase alongside it.
...
Instead: stop, move the ticket to `Blocked` — write the local ticket file's `status` to `blocked`, the same mechanism as step 2/10 — with a clear note on the ticket of what's blocking (see "Writing on the ticket"), and invoke the `triage` agent with the specifics of the blocker. Do not attempt to resolve it yourself beyond that.
