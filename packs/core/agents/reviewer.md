...
4. **Verification** — run `{{ project.checkCommand }}` and report actual output{{#if project.typecheckCommands}}; run {{ project.typecheckCommands | codelist }} if any type moved{{/if}}.
